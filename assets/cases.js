...
(function () {
  ...
  const CASES = {
    ...
    escaperoom: {
      ...
      sections: [
        ...
        { label: 'The engineering', h: 'Building a room that travels.', body: ["On the engineering side, I designed the puzzles and 3D models and leveraged hardware systems through Node-RED, all while making the whole experience fit inside a trailer."], viz: 'trailer', vizColor: '#4f86ff', vizCap: 'wire cutaway \u00b7 drag to spin \u00b7 puzzle stations, front console & the swing doors' },
    ...
    seeusnow: {
      ...
      sections: [
        ...
        { label: 'The reach', h: 'From Tampa to 48 states \u2014 and across the world.', viz: 'globe', vizColor: '#2fb36b', vizCap: 'live diagram \u00b7 drag to spin \u00b7 arcs: awareness-week reach by state, plus the UN (NYC), WHO (Bogot\u00e1) & Jamestown (South Africa)' },
    ...
    smif: {
      ...
      sections: [
        ...
        { label: 'FundPrep', body: ["FundPrep is our video series from FundraLabs \u2014 the research and investing playbook we teach from, episode by episode."], embed: 'https://www.youtube.com/embed/videoseries?list=PLaXiwiDM-VaQ' },
  ...
  function render() {
    ...
    let secHtml = c.sections.map((s, i) => {
      ...
      if (s.viz) inner += `<div class="cs-viz reveal" data-viz="${s.viz}" data-color="${s.vizColor || ''}"><canvas></canvas>${s.vizCap ? `<div class="viz-cap">${s.vizCap}</div>` : ''}</div>`;
    ...
    if (window.CaseViz) CaseViz.mountAll();
